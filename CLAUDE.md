...
4. **Private working docs live outside this repo** at `~/Documents/Claude/Projects/Career Progression/kyleries-site-2026-09-08/` — `HANDOFF.md` (session state and next steps), `REVIEW_NOTES.md` (every claim in every post, tagged by source), `field_notes_series_plan.md`, `private-checks/`, `voice-samples/` (Kyle's own unassisted writing), and `KYLE_VOICE_GUIDE.md` (the working guide to his register; read it before drafting anything in his name and run its checklist over every draft). **Read `HANDOFF.md` at the start of every session.** Never copy, quote, or summarize those files into this repo or into a commit message.
...
7. **Voice.** Follow `KYLE_VOICE_GUIDE.md` in the private folder. In short: first person, plain, specific, mechanistic. Open with the situation, not a thesis; headers are plain nouns; lists are for questions, resources, and components, never the argument; show an artifact; close by stepping back. No antithesis for effect ("not X; it is Y"), no aphorisms, at most one light aside. Numbers where allowed; no disparagement of what existed before Kyle arrived; "I led the platform work" and "my team and I", never a claim of authority above the actual role. Each field note ends with a short "What I'd carry into the next role" section — that is the sentence a hiring manager is reading for. No emoji, no hype, no listicles.
